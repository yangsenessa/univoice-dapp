...
pub async fn get_access_token(wallet_principal: String, bucket_id: String) -> Result<(String, String), String> {
    ...
    let (result,): (Result<Vec<u8>, String>,) = call(audience,
    ...
    match result {
        Ok(access_token) => {
            let access_token_str = hex::encode(access_token);
            Ok((access_token_str, "0/".to_string()))
        }
        Err(err) => Err(format!("Failed to get access token: {:?}", err)),
    }
}
